...
TEMPLATE_SCORING_MATRIX = {
    ...
    "constraint_clarity": 0.9,  # Are constraints or rules clearly defined?
    "reasoning_scope": 0.85,  # Does the prompt support traceable reasoning?
    "evalability": 0.85,  # Is the output evaluable?
    "ambiguity_avoidance": 0.75,  # Does the prompt avoid ambiguous phrasing?
    "domain_alignment": 0.7,  # Is the prompt aligned with its domain (e.g., B2B context)?
    "composability": 0.65,  # Is the prompt modular and extendable?
    "user_alignment": 0.9,  # Is the prompt aligned with the intended user need?
}
